...
from gui import GUI_PyGame as GuiModule, check_for_event, wait_for_event
...
def handle_event(event, digit, digits, numpad):
    ...
    if event.type == 2 and event.value[0] == 1:
        ...
        for num, pos in numpad.items():
            if (pos[0] < event.value[1][0] < pos[0] + button_size[0] and
                            pos[1] < event.value[1][1] < pos[1] + button_size[1]):
                digits[digit] = num
                return True
    return False
